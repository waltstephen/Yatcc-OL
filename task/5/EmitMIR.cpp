...
private:
  ...
  */
  ...
  void emitBinary(const llvm::BinaryOperator& bo)
  {
    ...
    switch (bo.getOpcode()) {
      ...
      case llvm::Instruction::Shl:
        if (const auto* imm = llvm::dyn_cast<llvm::ConstantInt>(bo.getOperand(1))) {
          emitVSlli(dst, lhs, imm->getZExtValue() & 63);
        } else {
      ...
      case llvm::Instruction::LShr:
        if (const auto* imm = llvm::dyn_cast<llvm::ConstantInt>(bo.getOperand(1))) {
          emitVSrli(dst, lhs, imm->getZExtValue() & 63);
        } else {
      ...
      case llvm::Instruction::AShr:
        if (const auto* imm = llvm::dyn_cast<llvm::ConstantInt>(bo.getOperand(1))) {
          emitVSrai(dst, lhs, imm->getZExtValue() & 63);
        } else {
  ...
  void emitLoadInst(const llvm::LoadInst& li)
  {
    ...
    uint64_t bytes = dl_.getTypeStoreSize(li.getType());
    if (bytes == 4) {
  ...
  void emitStoreInst(const llvm::StoreInst& si)
  {
    ...
    llvm::Register src = emitLoadValue(value);
    llvm::Register addr = emitLoadValue(si.getPointerOperand());
    uint64_t bytes = dl_.getTypeStoreSize(value->getType());
    if (bytes == 4) {
      emitVStore32(src, addr);
    } else {
      emitVStore(src, addr);
    }
  }
